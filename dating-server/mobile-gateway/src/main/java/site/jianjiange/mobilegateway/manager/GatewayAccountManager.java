...
public class GatewayAccountManager {
    ...
    public GatewayAccountEntity createDeviceAccount(String accountKeyHash, int hashVersion, OffsetDateTime now) {
        return createAccount(AuthConstants.ACCOUNT_TYPE_DEVICE, accountKeyHash, hashVersion, now);
    }

    /**
     * 创建手机号登录专用账号，并生成 account_id 和 user_id。
     *
     * @param accountKeyHash 手机号 HMAC hash
     * @param hashVersion HMAC hash 版本
     * @param now 当前时间
     * @return 新创建的账号实体
     */
    public GatewayAccountEntity createPhoneAccount(String accountKeyHash, int hashVersion, OffsetDateTime now) {
        return createAccount(AuthConstants.ACCOUNT_TYPE_PHONE, accountKeyHash, hashVersion, now);
    }
     ...
     * @return 新创建的账号实体
     */
    private GatewayAccountEntity createAccount(String accountType, String accountKeyHash, int hashVersion,
                                               OffsetDateTime now) {
